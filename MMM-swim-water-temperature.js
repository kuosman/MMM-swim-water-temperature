...
Module.register("MMM-swim-water-temperature", {
  ...
  getDom: function () {
    ...
    if (this.sensorData.error !== null && this.sensorData.error === true) {
      wrapper.innerHTML = this.translate('errorGettingData') + this.config.sensor;
      wrapper.className = 'swim-water-temperature dimmed light small';
    } else {
  ...
  scheduleNextFetch: function () {
    ...
    if (self.sensorData === null) {
      self.sendSocketNotification('GET_SENSOR_DATA_' + self.config.sensor, self.config);
    } else {
  ...
  socketNotificationReceived: function (notification, payload) {
    switch (notification) {
      case 'SENSOR_RESPONSE_' + this.config.sensor:
        this.scheduleNextFetch();
